...
describe("frontend presentational rendering", () => {
  ...
  it("renders all category filter options with the selected category marked active", () => {
    const html = renderToStaticMarkup(<CategoryFilter selectedCategory="developer_tools" onChange={() => undefined} />);

    expect(html).toContain("All");
    expect(html).toContain("Developer Tools");
    expect(html).toContain("AI Models");
    expect(html).toContain("bg-[#174936]");
  });

  it("renders source list fallback and generated timestamp content", () => {
    const emptyHtml = renderToStaticMarkup(<SourceList sources={[]} />);
    const populatedHtml = renderToStaticMarkup(
      <SourceList sources={["Mock AI Lab Blog"]} generatedAt="2026-04-22T12:00:00.000Z" />
    );
